...
func getImportance(employees []*Employee, id int) int {
	m := make(map[int]*Employee,len(employees))
...
func helper(data map[int]*Employee, id int) int {
	res := data[id].Importance
	for _, element := range data[id].Subordinates {
		res += helper(data,element)
	}
	return res
}
...
and employee 2 is the leader of employee 3. They have importance value 15, 10 and 5,
